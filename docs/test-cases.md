...
**测试文件
**: [JwtUtilsTest.java](file:///d:/PRJ/javaprj/class_times_record/src/test/java/com/shiroko/util/JwtUtilsTest.java)

| 编号        | 用例名称                  | 前置条件        | 输入                     | 预期结果                                               | 类型 |
|-----------|-----------------------|-------------|------------------------|----------------------------------------------------|----|
| TC-JWT-01 | 创建Token应返回非空字符串       | 无           | userId=1, roleId=2     | Token 非空且非空字符串                                     | 功能 |
| TC-JWT-02 | 创建Token应包含三段式JWT格式    | 无           | userId=1, roleId=2     | Token 按 `.` 分割得 3 段                                | 功能 |
| TC-JWT-03 | 有效Token校验应返回true      | 已生成合法 Token | 合法 Token               | `validateToken` 返回 `true`                          | 功能 |
...
| TC-JWT-07 | 解析有效Token应返回正确Claims  | 已生成合法 Token | userId=100, roleId=200 | Claims 含 subject="user_auth"，userId=100，roleId=200 | 功能 |
...
**测试文件
**: [RecordServiceImplTest.java](file:///d:/PRJ/javaprj/class_times_record/src/test/java/com/shiroko/service/impl/RecordServiceImplTest.java)
...
**测试文件
**: [CourseRecordServiceImplTest.java](file:///d:/PRJ/javaprj/class_times_record/src/test/java/com/shiroko/service/impl/CourseRecordServiceImplTest.java)

**被测方法
**: `addCourseRecord`, `deleteCourseRecord`, `updateCourseRecord`, `deductByStudentId`, `getCourseRecords`, `newGetCourseRecords`
...
**测试文件
**: [StudentServiceImplTest.java](file:///d:/PRJ/javaprj/class_times_record/src/test/java/com/shiroko/service/impl/StudentServiceImplTest.java)

**被测方法
**: `insertStudent`, `updateStudent`, `getStudent`, `getStudentByStudentId`, `getStudentByParentId`, `getStudentByTeacherId`, `getStudentByClassId`
...
| 模块                      | 方法数    | 已测     | 覆盖场景                                     |
|-------------------------|--------|--------|------------------------------------------|
| JwtUtils                | 4      | 4      | 正常 · 异常 · 边界(null/空/篡改)                  |
| RecordServiceImpl       | 3      | 3      | 单条插入 · 批量消课 · 批量增加 · 失败 · 分页(有/无数据)      |
| CourseRecordServiceImpl | 6      | 6      | 增删改 · 扣课(成功/聚合/余额不足) · 查询(权限注入/VO转换/空结果) |
| StudentServiceImpl      | 7      | 7      | 插入(单/双家长) · 更新 · 4种查询维度 · 已知识别(null返回)   |
| **合计**                  | **20** | **20** | **37 条用例**                               |
...
| 字段      | 约定值                  | 说明                          |
|---------|----------------------|-----------------------------|
| 当前用户 ID | `userId=1, roleId=2` | CourseRecordServiceImpl 中模拟 |
