...
│       ├── mapper/        # 公用 Mapper 接口（InstitutionMapper、StudentMapper 等）+ XML
│       │   └── resources/mapper/  # 公用 Mapper XML 文件
...
│       ├── mapper/        # 公用 Mapper 在 common 包，本目录已无业务 Mapper
...
│       ├── controller/    # SysUserController、SysRoleController、SysMenuController、SysDashboardController、AdminBusinessController
...
- **公用 Mapper**（被多个服务使用的）必须放在 `common/src/main/java/com/shiroko/mapper/` 下，对应的 XML 文件放在 `common/src/main/resources/mapper/` 下
- **服务专属 Mapper**（仅单个服务使用的）放在对应微服务的 `mapper/` 包下
- **命名规范**：Mapper 接口名 = 实体类名 + `Mapper`（如 `InstitutionMapper`、`StudentMapper`）
...
- Controller 只调用 Service 接口，不得直接注入 Mapper
...
| `teacher` | Teacher | 教师（继承 RoleBaseEntity） | teacher_id, institution_id, is_available, username |
| `student` | Student | 学生（继承 RoleBaseEntity） | id, student_name, institution_id, sex(0女/1男), birth, school, address |
| `parent` | Parent | 家长（继承 RoleBaseEntity） | id, parent_name, phone |
...
| 表名 | 实体类 | 说明 | 关键字段 |
|------|--------|------|----------|
| `user_auth` | UserAuth | 用户认证（账号密码） | id, user_id, role_id, account, password(SM3), salt |
| `user` | User | 小程序用户 | id, nickname, avatar, phone |
| `user_platform` | UserPlatform | 用户平台关联 | id, user_id, platform, platform_id |
...
| `parent_student` | ParentStudent | 家长-学生关联（多对多） | id, parent_id, student_id |
...
| 表名 | 实体类 | 说明 | 关键字段 |
|------|--------|------|----------|
| `sys_user` | SysUser | 管理员用户 | id, username, nickname, password(SM3), salt, phone, email, avatar, status, is_deleted(逻辑删除) |
...
```
BaseEntity (空基类，无字段)
└── RoleBaseEntity (teacher_id / institution_id / is_available / username)
    ├── Teacher
    ├── Student
    └── Parent
```
...
parent      N──N student        (parent_student 关联表)
user_auth   1──1 teacher/student/parent  (user_id + role_id 关联)
...
| role_id | 角色 | user_id 对应 |
|---------|------|-------------|
| 1 | 教师 | teacher.teacher_id |
| 2 | 家长 | parent.id |
| 3 | 学生 | student.id |
...
- 地址：121.196.229.10:3306
- 数据库名：class_times_record
- 用户名：class_times_record
- 密码：8BCnbZjTT8ZxmBj6
- 字符集：utf8
- JDBC URL：`jdbc:mysql://121.196.229.10:3306/class_times_record?useSSL=false&serverTimezone=UTC&characterEncoding=utf8`
- Nacos 配置中心 Data ID `common-db.yaml` 包含完整数据库连接配置
